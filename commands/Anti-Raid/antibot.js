...
module.exports = {
    ...
    //aliases: [''],
    category: 'Anti-Raid',
    description: 'Renvoit le panel de configuration de l\'antibot',
    usage: '',
    antiraid: true,
    ...
    run : async (client, message, args) => {        
        ...
        try {
            ...
            .setColor('#2f3136')
            .setTitle('AntiBot')
            .setDescription(`\`🔑\` Statut ➜ ${db.get(`guild_${message.guild.id}_statutAntiBot`) ? `${emojis.bc_on}` : `${emojis.bc_off}`}\n\`🧤\` Sanction ➜ ${db.get(`guild_${message.guild.id}_sanctionAntiBot`) ? `**Kick**` : `**Ban**`}\n\`🔒\` Réinitialiser`)
            .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
            .setTimestamp()
            ...
            collectorReaction.on('collect', async reaction => {
                switch (reaction.emoji.name || reaction.emoji.id) {
                    case '🔑':
                        const verifyStatut = await db.get(`guild_${message.guild.id}_statutAntiBot`, true);
                        if(verifyStatut) {
                            db.delete(`guild_${message.guild.id}_statutAntiBot`);
                        } else {
                            db.set(`guild_${message.guild.id}_statutAntiBot`, true);
                        }
                    ...
                    break;
                    case '🧤':
                        const verifySanction = await db.get(`guild_${message.guild.id}_sanctionAntiBot`, true);
                        if(verifySanction) {
                            db.delete(`guild_${message.guild.id}_sanctionAntiBot`);
                        } else {
                            db.set(`guild_${message.guild.id}_sanctionAntiBot`, true);
                        }
                    ...
                    break;
                    case '🔒':
                        const verifyStatutRéinitialisation = await db.get(`guild_${message.guild.id}_statutAntiBot`, true);
                        if(verifyStatutRéinitialisation) {
                            db.delete(`guild_${message.guild.id}_statutAntiBot`);
                        }

                        const verifySanctionRéinitialisation = await db.get(`guild_${message.guild.id}_sanctionAntiBot`, true);
            ...
            .setDescription(`Une erreur dans la commande \`antibot\` est survenue dans **${message.guild.name}**.`)
