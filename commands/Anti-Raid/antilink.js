...
module.exports = {
    ...
    //aliases: [''],
    category: 'Anti-Raid',
    description: 'Renvoit le panel de configuration de l\'antilink',
    usage: '',
    antiraid: true,
    ...
    run : async (client, message, args) => {        
        ...
        try {
            ...
            .setColor('#2f3136')
            .setTitle('AntiLink')
            .setDescription(`\`🔑\` Statut ➜ ${db.get(`guild_${message.guild.id}_statutAntiLink`) ? `${emojis.bc_on}` : `${emojis.bc_off}`}\n\`🧤\` Sanction ➜ ${db.get(`guild_${message.guild.id}_sanctionAntiLink`) ? `**Kick**` : `**Ban**`}\n\`🔒\` Réinitialiser`)
            .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
            .setTimestamp()
            ...
            collectorReaction.on('collect', async reaction => {
                switch (reaction.emoji.name || reaction.emoji.id) {
                    case '🔑':
                        const verifyStatut = await db.get(`guild_${message.guild.id}_statutAntiLink`, true);
                        if(verifyStatut) {
                            db.delete(`guild_${message.guild.id}_statutAntiLink`);
                        } else {
                            db.set(`guild_${message.guild.id}_statutAntiLink`, true);
                        }
                    ...
                    break;
                    case '🧤':
                        const verifySanction = await db.get(`guild_${message.guild.id}_sanctionAntiLink`, true);
                        if(verifySanction) {
                            db.delete(`guild_${message.guild.id}_sanctionAntiLink`);
                        } else {
                            db.set(`guild_${message.guild.id}_sanctionAntiLink`, true);
                        }
                    ...
                    break;
                    case '🔒':
                        const verifyStatutRéinitialisation = await db.get(`guild_${message.guild.id}_statutAntiLink`, true);
                        if(verifyStatutRéinitialisation) {
                            db.delete(`guild_${message.guild.id}_statutAntiLink`);
                        }

                        const verifySanctionRéinitialisation = await db.get(`guild_${message.guild.id}_sanctionAntiLink`, true);
            ...
            .setDescription(`Une erreur dans la commande \`antilink\` est survenue dans **${message.guild.name}**.`)
