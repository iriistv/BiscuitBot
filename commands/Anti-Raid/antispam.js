...
module.exports = {
    ...
    //aliases: [''],
    category: 'Anti-Raid',
    description: 'Renvoit le panel de configuration de l\'antispam',
    usage: '',
    antiraid: true,
    ...
    run : async (client, message, args) => {        
        ...
        try {
            ...
            .setColor('#2f3136')
            .setTitle('AntiSpam')
            .setDescription(`\`🔑\` Statut ➜ ${db.get(`guild_${message.guild.id}_statutAntiSpam`) ? `${emojis.bc_on}` : `${emojis.bc_off}`}\n\`🧤\` Sanction ➜ ${db.get(`guild_${message.guild.id}_sanctionAntiSpam`) ? `**Kick**` : `**Ban**`}\n\`🔒\` Réinitialiser`)
            .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
            .setTimestamp()
            ...
            collectorReaction.on('collect', async reaction => {
                switch (reaction.emoji.name || reaction.emoji.id) {
                    case '🔑':
                        const verifyStatut = await db.get(`guild_${message.guild.id}_statutAntiSpam`, true);
                        if(verifyStatut) {
                            db.delete(`guild_${message.guild.id}_statutAntiSpam`);
                        } else {
                            db.set(`guild_${message.guild.id}_statutAntiSpam`, true);
                        }
                    ...
                    break;
                    case '🧤':
                        const verifySanction = await db.get(`guild_${message.guild.id}_sanctionAntiSpam`, true);
                        if(verifySanction) {
                            db.delete(`guild_${message.guild.id}_sanctionAntiSpam`);
                        } else {
                            db.set(`guild_${message.guild.id}_sanctionAntiSpam`, true);
                        }
                    ...
                    break;
                    case '🔒':
                        const verifyStatutRéinitialisation = await db.get(`guild_${message.guild.id}_statutAntiSpam`, true);
                        if(verifyStatutRéinitialisation) {
                            db.delete(`guild_${message.guild.id}_statutAntiSpam`);
                        }

                        const verifySanctionRéinitialisation = await db.get(`guild_${message.guild.id}_sanctionAntiSpam`, true);
            ...
            .setDescription(`Une erreur dans la commande \`antispam\` est survenue dans **${message.guild.name}**.`)
