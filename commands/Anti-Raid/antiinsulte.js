...
module.exports = {
    ...
    //aliases: [''],
    category: 'Anti-Raid',
    description: 'Renvoit le panel de configuration de l\'antiinsulte',
    usage: '',
    antiraid: true,
    ...
    run : async (client, message, args) => {        
        ...
        try {
            ...
            .setColor('#2f3136')
            .setTitle('AntiInsulte')
            .setDescription(`\`🔑\` Statut ➜ ${db.get(`guild_${message.guild.id}_statutAntiInsulte`) ? `${emojis.bc_on}` : `${emojis.bc_off}`}\n\`🧤\` Sanction ➜ ${db.get(`guild_${message.guild.id}_sanctionAntiInsulte`) ? `**Kick**` : `**Ban**`}\n\`🔒\` Réinitialiser`)
            .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
            .setTimestamp()
            ...
            collectorReaction.on('collect', async reaction => {
                switch (reaction.emoji.name || reaction.emoji.id) {
                    case '🔑':
                        const verifyStatut = await db.get(`guild_${message.guild.id}_statutAntiInsulte`, true);
                        if(verifyStatut) {
                            db.delete(`guild_${message.guild.id}_statutAntiInsulte`);
                        } else {
                            db.set(`guild_${message.guild.id}_statutAntiInsulte`, true);
                        }
                    ...
                    break;
                    case '🧤':
                        const verifySanction = await db.get(`guild_${message.guild.id}_sanctionAntiInsulte`, true);
                        if(verifySanction) {
                            db.delete(`guild_${message.guild.id}_sanctionAntiInsulte`);
                        } else {
                            db.set(`guild_${message.guild.id}_sanctionAntiInsulte`, true);
                        }
                    ...
                    break;
                    case '🔒':
                        const verifyStatutRéinitialisation = await db.get(`guild_${message.guild.id}_statutAntiInsulte`, true);
                        if(verifyStatutRéinitialisation) {
                            db.delete(`guild_${message.guild.id}_statutAntiInsulte`);
                        }

                        const verifySanctionRéinitialisation = await db.get(`guild_${message.guild.id}_sanctionAntiInsulte`, true);
            ...
            .setDescription(`Une erreur dans la commande \`antiinsulte\` est survenue dans **${message.guild.name}**.`)
