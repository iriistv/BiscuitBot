...
module.exports = {
    ...
    //aliases: [''],
    category: 'Anti-Raid',
    description: 'Renvoit le panel de configuration de l\'antinewaccount',
    usage: '',
    antiraid: true,
    ...
    run : async (client, message, args) => {        
        ...
        try {
            ...
            .setColor('#2f3136')
            .setTitle('AntiNewAccount')
            .setDescription(`\`🔑\` Statut ➜ ${db.get(`guild_${message.guild.id}_statutAntiNewAccount`) ? `${emojis.bc_on}` : `${emojis.bc_off}`}\n\`🧤\` Sanction ➜ ${db.get(`guild_${message.guild.id}_sanctionAntiNewAccount`) ? `**Kick**` : `**Ban**`}\n\`🔒\` Réinitialiser`)
            .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
            .setTimestamp()
            ...
            collectorReaction.on('collect', async reaction => {
                switch (reaction.emoji.name || reaction.emoji.id) {
                    case '🔑':
                        const verifyStatut = await db.get(`guild_${message.guild.id}_statutAntiNewAccount`, true);
                        if(verifyStatut) {
                            db.delete(`guild_${message.guild.id}_statutAntiNewAccount`);
                        } else {
                            db.set(`guild_${message.guild.id}_statutAntiNewAccount`, true);
                        }
                    ...
                    break;
                    case '🧤':
                        const verifySanction = await db.get(`guild_${message.guild.id}_sanctionAntiNewAccount`, true);
                        if(verifySanction) {
                            db.delete(`guild_${message.guild.id}_sanctionAntiNewAccount`);
                        } else {
                            db.set(`guild_${message.guild.id}_sanctionAntiNewAccount`, true);
                        }
                    ...
                    break;
                    case '🔒':
                        const verifyStatutRéinitialisation = await db.get(`guild_${message.guild.id}_statutAntiNewAccount`, true);
                        if(verifyStatutRéinitialisation) {
                            db.delete(`guild_${message.guild.id}_statutAntiNewAccount`);
                        }

                        const verifySanctionRéinitialisation = await db.get(`guild_${message.guild.id}_sanctionAntiNewAccount`, true);
            ...
            .setDescription(`Une erreur dans la commande \`antinewaccount\` est survenue dans **${message.guild.name}**.`)
