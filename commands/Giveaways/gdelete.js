...
module.exports = {
    ...
    //aliases: [''],
    category: 'Giveaways',
    description: 'Permet de supprimer un giveaway',
    giveaways: true,
    ...
    run : async (client, message, args) => {
        ...
        try {
            ...
            .setColor('#2f3136')
            .setDescription(`${emojis.bc_cross} Veuillez fournir l'ID du giveaway à supprimer.`)
            .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
            .setTimestamp()
            )

            client.giveawaysManager.delete(giveawayID).then(() => {
                message.channel.send(new Discord.MessageEmbed()
                .setColor('#2f3136')
                .setDescription(`${emojis.bc_check} Votre giveaway a bien été supprimé.`)
                .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
                .setTimestamp()
                )
            }).catch((err) => {
            ...
            .setDescription(`Une erreur dans la commande \`gdelete\` est survenue dans **${message.guild.name}**.`)
