...
module.exports = {
    ...
    run : async (client, message, args) => {
        ...
        try {
            ...
            if(!message.client.player.getQueue(message).previousTracks[0]) return message.channel.send(new Discord.MessageEmbed()
            .setColor('#2f3136')
            .setDescription(`${emojis.bc_cross} Aucune musique n'a été détectée avant celle-ci.`)
            .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
            .setTimestamp()
            )
            ...
            .setColor('#2f3136')
            .setDescription(`${emojis.bc_check} La musique précédente est désormais en cours de lecture.`)
            .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
            .setTimestamp()
            )
        } catch(err) {
            ...
            .setDescription(`Une erreur dans la commande \`back\` est survenue dans **${message.guild.name}**.`)
