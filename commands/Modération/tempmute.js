...
module.exports = {
    ...
    //aliases: [''],
    category: 'Modération',
    description: 'Permet de mute un membre pendant un certains temps',
    modération: true,
    ...
    run : async (client, message, args) => {
        ...
        try {
            ...
            try {
                ...
                .setColor('#2f3136')
                .setDescription(`${emojis.bc_check} ${target} a bien été **mute** pendant **${humnizeDuration(time, { language: 'fr' })}**.`)
                .setFooter(`${client.user.username} © 2021`, client.user.displayAvatarURL({dynamic: true}))
                .setTimestamp()
                )
                setTimeout(() => {
                    if(target.deleted || !target.manageable) return;
                    target.roles.remove(muteRole, 'Unmute');
                }, time)
            ...
            .setDescription(`Une erreur dans la commande \`tempmute\` est survenue dans **${message.guild.name}**.`)
